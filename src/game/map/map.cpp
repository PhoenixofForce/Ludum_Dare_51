...
GameMap::GameMap(int inWindowWidth, int inWindowHeight, const std::string& name) 
: scale(0.5f), windowWidth(inWindowWidth), windowHeight(inWindowHeight)
{

    if(name.length() > 0) {
        loadMap("res/" + name + ".map", 
            [this](int tileSize, std::vector<Tag> tags) { 
                this->tileSize = tileSize;
                for(const Tag& tag: tags) {
                    if(tag.header == "import") {
                        texture::loadSpriteSheetPng(tag.body);
                        textureSheet = tag.body;
                    }
                }    
            },
            [this](IDMapper mapper, float d, Matrix_m<int> map) { },
...
void GameMap::render(vec::vec2f cameraPosition) {
    //face
    drawImage(entities[0].sprite.getTexture(), entities[0].position - cameraPosition, scale);

    //outline
    for(int i = 1; i < entities.size(); i++) {
        const Entity& e{ entities.at(i) };
        drawImage(e.sprite.getTexture() + "_outline", e.position - cameraPosition, scale);
    }

    //hair
    for(int i = 1; i < entities.size(); i++) {
        const Entity& e{ entities.at(i) };
        drawImage(e.sprite.getTexture(), e.position - cameraPosition, scale);
    }
